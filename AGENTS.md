...
│   │       ├── agents/              # 13 agents, 12 createXXXAgent factories (Prometheus special-cased via plugin-handlers/prometheus-agent-config-builder.ts)
...
- `hard-reject`: oracle, librarian, explore, multimodal-looker, metis, momus, praha, prometheus (rejected at parse — use `task`/delegate-task)
