...
describe("omo-senpi package shape", () => {
  test("#given the senpi adapter manifest #when audited #then it declares the package contract", async () => {
    ...
    expect(peerDependencies["@code-yeongyu/senpi"]).toBe("2026.9.3")
    expect(peerDependenciesMeta["@code-yeongyu/senpi"]).toMatchObject({ optional: true })
    expect(devDependencies["@code-yeongyu/senpi"]).toBe("2026.9.3")
    expect(rootPatchedDependencies["@code-yeongyu/senpi@2026.9.3"]).toBe(
      "patches/@code-yeongyu%2Fsenpi@2026.9.3.patch",
    )
