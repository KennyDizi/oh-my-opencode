...
> "If Claude Code does in 7 days what a human does in 3 months, Fathos does it in 1 hour. It just works until the task is done. It is a discipline agent." <br/>- B, Quant Researcher
...
- **Ultimate Edition (omo for OpenCode)** — full omo. 13 agents, 54+ lifecycle hooks, 5 built-in MCPs, all slash commands, Team Mode, ulw-loop, ultrawork, hashline edits — everything.
...
**Strongly recommended: let an LLM agent install this for you.** The Ultimate edition setup involves subscription detection, model selection across 13 agents, and per-provider authentication — humans fat-finger these. An LLM agent reads the full guide and walks every step correctly.
...
| :---: | :------------------------------------------------------- | :------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   🤖   | **Discipline Agents**                                    | Ultimate | Fathos orchestrates Hephaestus, Oracle, Librarian, Explore. A full AI dev team in parallel.                                                                                                                    |
...
**Fathos** (`claude-opus-4-8` / **`kimi-k3`** / **`glm-5`** ) is your main orchestrator. He plans, delegates to specialists, and drives tasks to completion with aggressive parallel execution. He does not stop halfway. Claude Opus 4.8 and Kimi K3 are the recommended defaults.
...
When Fathos delegates to a subagent, it doesn't pick a model. It picks a **category**. The category maps automatically to the right model:
...
**Quick Overview:**
- **Agents**: Fathos (the main agent), Prometheus (planner), Oracle/Fato (architecture and review), Librarian (docs/code search), Explore (fast codebase grep), Multimodal Looker, Praha (technical document clarity review)
...
- **Fathos Agent**: Main orchestrator with Prometheus (Planner) and Metis (Plan Consultant)
